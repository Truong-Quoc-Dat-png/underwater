...
def process_image(request, image_id):
    ...
    pil_img = PILImage.open(img.image.path)
    
    np_image = np.array(pil_img)

    imgx = cv2.resize(np_image,(256,256))
    ...
    # Chuyển đổi mảng NumPy thành hình ảnh Pillow
    image = PILImage.fromarray((imgx).astype(np.uint8))
...
def download_image(request, image_id):
    ...
    with open(img.image.path, 'rb') as f:
        ...
        response['Content-Disposition'] = f'attachment; filename="{img.title}.png"'
        return response
